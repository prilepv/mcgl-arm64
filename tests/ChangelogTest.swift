...
struct ChangelogTest {
    static func main() {
        ...
        precondition(entries.map(\.version) == ["1.7.2", "1.7.1", "1.7.0", "1.6.7", "1.6.6", "1.6.5", "1.6.4"])
        ...
        preferences.lastReadChangelogVersion = "1.7.2"
        precondition(MCGLLauncherPreferences(defaults: defaults).lastReadChangelogVersion == "1.7.2")
        precondition(preferences.maximumMemoryMB == 4096 && preferences.fpsLimit == 144)
        print("CHANGELOG_PASS seven concise public releases, descending versions, exact HTTPS links, offline data, independent read marker")
    }
}
