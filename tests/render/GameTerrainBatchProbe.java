...
public final class GameTerrainBatchProbe {
    ...
    public static int run(GameRenderCommands g)throws Exception{
        boolean wide=Boolean.parseBoolean(System.getProperty("mcgl.terrain.wideArena","true"));
        int tags=wide?128:32,manyCount=tags+38,modelCount=manyCount+26;
        checks=0;int first=g.glGenLists(3*modelCount),previousModels=g.cachedModels(),previousMembers=g.terrainArenaMembers();
        ...
        try{
            ...
            int[] twelve=handles(first,12),many=handles(first,manyCount);
            ...
            calls=compareIndividual(g,"individual world lists with external camera transforms",twelve,()->{});
            check(calls==1,"the original alpha-sort individual-list path uses one ordered multi-submission");
            int[] reversed=twelve.clone();reverse(reversed);
            compareIndividual(g,"reverse transparent individual world lists",reversed,()->g.glEnable(GL11C.GL_BLEND));
            compareIndividual(g,"duplicate individual world lists under different transforms",new int[]{first,first+3,first,first+6,first},()->g.glEnable(GL11C.GL_BLEND));
            compareIndividual(g,"large camera offset individual world lists",twelve,()->{g.glTranslated(-(1<<24),1<<24,-8192);g.glTranslated(1<<24,-(1<<24),8192);});
            ...
            calls=compare(g,"more distinct matrices than the configured palette",many,()->g.glEnable(GL11C.GL_BLEND));check(calls==(manyCount+tags-1)/tags,"palette collisions split runs without losing batching");
            calls=compare(g,"two live meshes sharing a recycled palette tag",new int[]{first,first+tags*3},()->g.glEnable(GL11C.GL_BLEND));check(calls==2,"conflicting live tag values cannot overwrite an earlier matrix");
            long creations=g.terrainArenaCreations(),bytes=g.terrainArenaBytes();
            for(int turn=0;turn<8;turn++){final int camera=turn;int[] selected=Arrays.copyOfRange(many,turn,manyCount-turn);if((turn&1)!=0)reverse(selected);compare(g,"sparse camera selection "+turn,selected,()->g.glTranslatef(camera*.013f,-camera*.019f,0));}
            ...
            int interleaved=first+manyCount*3;
            for(int i=0;i<6;i++){
            ...
            int ordinary=first+(manyCount+6)*3;g.glNewList(ordinary,4864);emit(t,5,7);g.glEndList();
            ...
            int nested=first+(manyCount+7)*3;g.beginOriginalChunk(nested,3);g.glNewList(nested,4864);g.glPushMatrix();g.glTranslatef(-.2f,0,0);g.glCallList(first);g.glTranslatef(.4f,0,0);g.glCallList(first);g.glPopMatrix();g.glEndList();g.finishOriginalChunk();
            ...
            reset(g);clear(g);
            rejects(()->{int scope=g.beginOriginalTerrain();try{g.glCallList(first);g.glCallList(first+3);g.glCallList(malformed);}finally{g.endOriginalTerrain(scope);}});
            check(Arrays.equals(prefix,pixels(g)),"failed individual world sequence preserves its completed prefix");
            reset(g);clear(g);int outer=g.beginOriginalTerrain(),inner=g.beginOriginalTerrain();
            rejects(()->g.endOriginalTerrain(outer));g.glCallLists(IntBuffer.wrap(twelve));g.endOriginalTerrain(inner);g.endOriginalTerrain(outer);
            rejects(()->g.endOriginalTerrain(outer));
            int[] scopes=new int[64];for(int i=0;i<64;i++)scopes[i]=g.beginOriginalTerrain();
            rejects(()->g.beginOriginalTerrain());for(int i=63;i>=0;i--)g.endOriginalTerrain(scopes[i]);
            ...
            for(int mode:new int[]{4,5,6,7,8}){
                int h=first+(manyCount+16)*3;for(int i=0;i<2;i++){g.beginOriginalChunk(h+i*3,3);g.glNewList(h+i*3,4864);g.glPushMatrix();g.glTranslatef(i*.2f,0,0);emit(t,i,mode);g.glPopMatrix();g.glEndList();g.finishOriginalChunk();}
                compare(g,"original primitive topology "+mode,new int[]{h,h+3},()->g.glEnable(GL11C.GL_BLEND));g.glDeleteLists(h,6);
            }
            check(g.terrainArenaBytes()<=(wide?320L:256L)*1024*1024&&g.terrainArenaMembers()>0,"original shared storage remains bounded");check(g.glGetError()==0,"original model batching is Core-valid");
        }finally{
            g.selectEffect(null);g.abortOriginalChunk();g.glDeleteLists(first,3*modelCount);if(textureA!=0)g.glDeleteTextures(textureA);if(textureB!=0)g.glDeleteTextures(textureB);
    ...
    private static long compare(GameRenderCommands g,String label,int[] names,Runnable setup){
        ...
        check(Arrays.equals(expected,pixels(g)),label+" exact RGBA");check(Arrays.equals(expectedDepth,depthPixels(g)),label+" exact depth buffer");check(Arrays.equals(matrix,matrix(g))&&Arrays.equals(projection,projection(g))&&Arrays.equals(current,color(g))&&depth==g.glGetBoolean(GL11C.GL_DEPTH_WRITEMASK),label+" final logical/raster state");check(selected.position()==1&&selected.limit()==names.length+1,label+" caller cursor");
        reset(g);clear(g);setup.run();int scope=g.beginOriginalTerrain();
        try{for(int name:names)g.glCallList(name);}finally{g.endOriginalTerrain(scope);}
        check(Arrays.equals(expected,pixels(g)),label+" individual scope exact RGBA");check(Arrays.equals(expectedDepth,depthPixels(g)),label+" individual scope exact depth");
        check(Arrays.equals(matrix,matrix(g))&&Arrays.equals(projection,projection(g))&&Arrays.equals(current,color(g))&&depth==g.glGetBoolean(GL11C.GL_DEPTH_WRITEMASK),label+" individual scope final state");
        return calls;
    }
    private static long compareIndividual(GameRenderCommands g,String label,int[] names,Runnable setup){
        reset(g);clear(g);setup.run();individual(g,names);byte[] expected=pixels(g),expectedDepth=depthPixels(g);float[] expectedMatrix=matrix(g);
        reset(g);clear(g);setup.run();long before=g.chunkDrawCalls();int scope=g.beginOriginalTerrain();
        try{individual(g,names);}finally{g.endOriginalTerrain(scope);}
        long calls=g.chunkDrawCalls()-before;
        check(Arrays.equals(expected,pixels(g)),label+" exact RGBA");check(Arrays.equals(expectedDepth,depthPixels(g)),label+" exact depth");check(Arrays.equals(expectedMatrix,matrix(g)),label+" final matrix");
        return calls;
    }
    private static void individual(GameRenderCommands g,int[] names){
        for(int i=0;i<names.length;i++){
            g.glPushMatrix();g.glTranslated((i%4)*.03125-.0625,(i%3)*-.015625,.0078125*i);g.glCallList(names[i]);g.glPopMatrix();
        }
    }
